...
if opt.finetune == '' then -- build network from scratch
  ...
  -- CrossEntropy Criterion Net
  net2 = nn.Sequential()
  net2:add(nn.SpatialConvolution(3,64,11,11,4,4,2,2))       -- 224 -> 55
  net2:add(nn.SpatialBatchNormalization(64,1e-3))
  net2:add(nn.ReLU(true))
  net2:add(nn.SpatialMaxPooling(3,3,2,2))                   -- 55 ->  27
  net2:add(nn.SpatialConvolution(64,192,5,5,1,1,2,2))       --  27 -> 27
  net2:add(nn.SpatialBatchNormalization(192,1e-3))
  net2:add(nn.ReLU(true))
  net2:add(nn.SpatialMaxPooling(3,3,2,2))                   --  27 ->  13
  net2:add(nn.SpatialConvolution(192,384,3,3,1,1,1,1))      --  13 ->  13
  net2:add(nn.SpatialBatchNormalization(384,1e-3))
  net2:add(nn.ReLU(true))
  net2:add(nn.SpatialConvolution(384,256,3,3,1,1,1,1))      --  13 ->  13
  net2:add(nn.SpatialBatchNormalization(256,1e-3))
  net2:add(nn.ReLU(true))
  net2:add(nn.SpatialConvolution(256,256,3,3,1,1,1,1))      --  13 ->  13
  net2:add(nn.SpatialBatchNormalization(256,1e-3))
  net2:add(nn.ReLU(true))
  net2:add(nn.SpatialMaxPooling(3,3,2,2))                   -- 13 -> 6
  net2:add(nn.View(256*6*6))
  net2:add(nn.Dropout(0.5))
  net2:add(nn.Linear(256*6*6, 4096))
...
local criterion = nn.ParallelCriterion():add(abs):add(cec)
